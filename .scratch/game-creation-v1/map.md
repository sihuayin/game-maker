...
- ⚠️ **2026-09-24 21:xx 实测：视觉路径现在通了**（第三次翻转，见下面被划掉的那条）。
  ...
  ~~这对「提取精确色板」是个真风险~~；
  🔴🔴 **2026-09-25 由票 04 的原型钉死：上游重编码图片 ⇒ 精确读色不可能。**
  送一张 **64×64 纯单色**图问它是什么颜色，5 次**零精确**，最差的 `#6b4a33` 被读成
  `#8e7c68`（ΔRGB=80.8，暗色被整体抬亮）。而票 04 特意造了三张**只有 18 色、
  每像素都恰好是其中之一**的图，提取出的 palette **精确命中 0/8、0/9、0/8，覆盖率 0.0%**。
  ⇒ 这**不是**参考图质量的问题（换一张干净的图一样），是上游在模型看到之前就转过一道。
  **「提取精确色板」不是有风险，是不可能。** 后果：`palette` 永远是近似 ——
  契约本来就只要求「颜色 ∈ 色板」（相对于 StyleSpec），从不要求它等于参考图。
  别再拿「色板准不准」评价参考图的好坏。证据：`experiments/reference-image-draft/`。

  🔴 **2026-09-25 由票 15 修正归因**：参考图 `fixtures/reference/test.png` 是**有损压缩的视频帧**
  （122,240 色、B 站水印与播放按钮在图上），**本来就没有「精确色板」可偏差** ——
  偏差不在读取，在于源图没有那个东西。且 `#708090`（一个与图无关的颜色）也能落在
  ΔRGB=3.2 内 ⇒ **「离得近」不构成读取正确的证据**；② **无法确认现在是谁在计费**，所以
...
- [交付物的版本化与校验和](issues/18-artifact-ref-consistency.md)：**八条裁决** ——
  **六条本票新定**、两条把已落定的决定收口（票 24 的 per-pack 粒度与逐文件 sha256）。
  新的是：**交付形态 = 只出目录，永不 zip**（zip 是「导出」动作，不是生成动作 ——
  逐文件校验和遇上 zip 会立刻产生「校验哪一个」的歧义）；
  **站点也版本化 `site/v<N>/`，用站点自己的计数器**（不跟包版本走 ——
  **外壳会变而包不变**，跟号就没处放），且**记录它消费的是哪个包版本**。
  站点**不做**逐文件校验和（它的主体是构建工具的产物，记的是工具链版本不是我们的内容）。
  收口的是：per-pack 版本粒度（票 24）· 逐文件 sha256（票 24）· `verify` 消费它且
  校验失败**是失败不是降级**（票 30）。**不加并发锁**，把「`outDir` 写入者必须串行」
  写成契约（落在 `nextPackVersion` 的注释里，连同 `renameSync` 要求同文件系统那条隐含前提）。
  **增量生成不留形状**，维持整包重建。
  🔴 **删掉 `ArtifactRefSchema` + `ArtifactRef` + `ArtifactType`**（零消费者、
  字段为已出局的闭环模型设计；`ArtifactType` 里 evaluation/repair-plan/benchmark 随 R2 出局）——
  **票面的问题 5、6（id 生成规则、path 相对/绝对）随之消解**，那两个问题今天没有对象。
  ⚠️ **票 29 记的 `out/<gameId>/{pack/v<N>, site}` 被本票取代**：site 现在也要版本号。
  **消解票 33 的一条**：它第 1 问里「zip 怎么办」不用答了。`CONTEXT.md` 的
  Artifact / ArtifactRef / 校验和 / Demo Site 四词已改。219 条测试全绿。

- [程序化造一张参考图（兼作 fixture 参考图）](issues/04-reference-image.md)：
  **交付 `fixtures/reference/halt-dusk.png`** —— 480×270、**18 色**、每一笔都是程序化画的，
  替代那张商业游戏视频截图当 fixture 参考图（版权 + 可验证）。人类拍板：构图取**分层剖面**
  （天空/候车室内景/站台三条带，各带一套光照），题材取**黄昏的山间列车小站**。
  造法 = **Node + 仓库自己的 `RasterImage`/`encodePNG`**，零第三方依赖、不需要浏览器。
  🔴 **本票最大的产出是一个否证**：为了「考验色板提取」把色板刻意做到 18 色、
  **每像素都恰好是其中之一**，提取出来**精确命中 0/8、0/9、0/8，覆盖率 0.0%**；
  再送一张 **64×64 纯单色**图问它是什么颜色，**5 次零精确**（最差 `#6b4a33`→`#8e7c68`，ΔRGB=80.8）。
  ⇒ **与图的色数/质量/格式无关，是上游重编码**。**「提取精确色板」不是有风险，是不可能**
  —— 推翻了票 15 把这件事归因于「那张图有 122,240 色」的推论。见「环境事实」。
  另：多图**只做单图**（票 15 实测：多图是重合成，不是交/并集）；分辨率**取原生**
  （实测 2× 最近邻与原生提取结果无实质差别）；**不造持久反例图**（票 15 已证 prompt 无偏）。

- [用参考图真跑一次 StyleSpec 提取，并固化为 fixture](issues/15-stylespec-fixture.md)：
  **产物 A 的入口现在通了** —— `fixtures/style-spec.json` 驱动 `pack` 出 8 资源 / 3 图集 / 28 文件，
  `verify` 全对。**视觉路径 thinking 关掉快 20 倍（2.9s vs 61s）而且更稳**（5/5 vs 2/3）
  —— 61s 那个数字一直是 thinking 开着的结果。
  🔴 **推翻了从票 01 传到地图的前提：参考图根本没有色板。** 那张图是 **122,240 色**的
  **B 站视频截图**（水印在图上），占比最高的色只有 1.42% ⇒「提取精确色板」是**构造**不是测量。
  模型的色板按「量化误差」**劣于随机取 9 个像素**（25.5 vs 22.8），但**最优解是九色全灰、
  画不出橙色招牌** —— **尺子量错了职责**。已毕业成**票 39**。
  两个坑：**原样输出过不了契约**（模型几乎总回大写，规范化小写后 1/5 → 5/5）；
  **`confidence` 会被漏掉**而它是唯二没有默认值的必填项。
  反例验证**通过**（霓虹图零重合）；多图既不是交集也不是并集，是**重合成**（与两边色板交集都是 0）
  ⇒ **V1 只做单图**。**`04 → 15` 这条边不成立**（图当时就有）—— 但 04 现在有了新理由：**版权**。
...
- [monorepo 切分与构建工具链](issues/29-monorepo-layout.md)：**pnpm workspaces + `exports` → `dist` +
  ...
  ⚠️ **2026-09-25：[票 18](issues/18-artifact-ref-consistency.md) 把 `site` 改成 `site/v<N>/`** —— 站点也要版本号。
...
- **新资源的风格一致性怎么判**：R2 砍掉了自动评分，于是「像不像同一个视觉世界」
  ...
  ⚠️ **「视觉上游不可用」这条已作废**（票 15 实测：9 次调用全 200，2.1–14.6 s），
  但**抽查仍只描述差异、不打分、不阻断**（票 22 的裁决），所以它给的仍不是判据。
  🔴 **2026-09-25 部分毕业**：票 15 发现「交付态每个色板色的像素占比」是**确定性、可计算**的
  （扫图集即可），而 v6 真包里玩家**只用了 5/9 色**、道具**只用 2/9**。
  那部分已成[票 39](issues/39-palette-role.md)。
  **剩下的雾**：色板之外的维度（形状语言、材质、光照）在交付态还是只有人眼 ——
  它们没有 `palette:N` 那样的构造性约束，也没有可扫的量。
